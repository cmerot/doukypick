...
<Card.Root>
	<Card.Content>
		{#if successMessage}
			<div
				class="mb-6 flex items-center space-x-3 rounded-lg bg-success p-4 text-success-foreground"
			>
		...
		<form method="POST" enctype="multipart/form-data" use:enhance class="space-y-8" novalidate>
			...
			<Form.Fieldset {form} name="projectTypeFlash">
				...
				<div class="space-y-2">
					<Label class="flex items-center space-x-2 font-normal">
						<input
							type="checkbox"
							name="projectTypeFlash"
							bind:checked={$formData.projectTypeFlash}
						/>
				...
				<Form.Control>
					{#snippet children({ props })}
						...
						<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
							<Form.Description class="order-1 prose md:order-2">
								<p>
									Décris-moi ici en quelques mots le motif que tu souhaites réaliser en tatouage!
								</p>
								<p>Exemples:</p>
								<ul>
								...
								<p>
									...
									choix/envies. Ton histoire fait partie de ton intimité et tu n'es pas forcé·e de
									la partager.
								</p>
								<p>
									Mais si tu es un petit peu perdu·e dans tes idées, n'hésite pas à me le dire,
									aussi je t'aiderai à affiner tes envies, pour créer un motif unique.
								</p>
								<p>
					...
					{#snippet children({ props })}
						...
						<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
							<Form.Description class="order-1 prose md:order-2">
								...
								<p>
									Pour un membre, dos, torse complet tu peux passer directement aux questions
									suivantes, t'ennuies pas ton double-décimètre du collège.
								</p>
							</Form.Description>
							<Input
								{...props}
				...
				<Form.Control>
					{#snippet children({ props })}
						...
						<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
							<Form.Description class="order-1 prose md:order-2">
								<p>
									Ex: "derrière mon oreille gauche", "Sur mollet droit", "De mon oreille gauche à
									mon mollet droit, en passant par mon nombril".
								</p>
							</Form.Description>
							<Input
								{...props}
				...
				<Form.Control>
					{#snippet children({ props })}
						...
						<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
							<Form.Description class="order-1 prose md:order-2">
								<p>
									Je respecte chaque porte-monnaie, je suis capable de faire des efforts si le
									projet me motive, mais je ne travaille pas à perte...
								</p>
								<p>
			...
			<Form.Fieldset {form} name="timeline">
				...
				<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
					<Form.Description class="order-1 prose md:order-2">
						<p>
							Tu souhaiterais tout ça pour quand ? (Attention je ne prépare jamais les dessins plus
